...
import SnakeHeader from './GameHeader'
import ScoreBoard from './ScoreBoard'
import SnakeBoard from './SnakeBoard'
import { Body, Snake } from '../../../utils/LinkedList'
import { TurningPoint } from '../../../utils/Queue'
...
class App extends Component {
  constructor() {
    ...
    this.state = {
      ...
      foodCoordinate: [8, 18],
      foodClass:
        this.foodClasses[this.getRandomInt(0, this.foodClasses.length - 1)],
  ...
  startGame(snakeDelay) {
    ...
    this.setState({
      gameMenuDisplay: false,
      gameOverDisplay: false,
      bestScore: this.getBestScore(this.mode),
    })

    this.intervalId = window.setInterval(this.updateSnakeState, this.snakeDelay)
  }
  ...
  onSwipeMove(position, event) {
    ...
    if (this.allowSwipe) {
      if (Math.abs(x) > tolerance || Math.abs(y) > tolerance) {
        ...
        else this.keyListner({ keyCode: x > 0 ? 39 : 37 })
      }
    }
  }
